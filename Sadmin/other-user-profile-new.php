...
<? } ?>
             ...
             <?  $part = mysqli_query($datos, "SELECT A.STSK_CHARGE_USR, CONCAT(B.USR_NAME, ' ', B.USR_SURNAME), A.STSK_ID, A.STSK_PROGRESS, A.STSK_ISS_ID FROM SUBTASKS A INNER JOIN USERS B ON(B.USR_ID = A.STSK_CHARGE_USR) WHERE (STSK_TYPE= 1 AND STSK_ISS_ID =" . ((int)$fila5[11]+1) . " AND STSK_CHARGE_USR != STSK_MAIN_USR)"); 
                                    ...
                                    mkdir("../" . $_SESSION['TxtFacility'] . "/" . $fint[0] . "_alt/", 0775, true); 
                                        ...
                                        if($handler2 = opendir("../" . $_SESSION['TxtFacility'] . "/" . $fint[0] . "_alt/" )){
                                          ...
                                          $file_extension = "";
                                           ...
                                           while (false !== ($archivos2 = readdir($handler2))){
echo "<script>console.info('tester : " . $archivos2 . "' + ' / ' + '" . preg_match_all("/_\[" . ($fint[4]+1). "\]_/", $archivos2) . "' + '/' + '" . ($fint[4] + 1). "' )</script>";
                                         if(preg_match_all("/_\[" . ($fint[4]+1) . "\]_/", $archivos2) == 1){
