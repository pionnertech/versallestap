...
switch ($c_range['RAN']) {
	...
	case 'back-user':
       ...
       $avg = mysqli_fetch_assoc(mysqli_query($datos, "SELECT ROUND(AVG(STSK_PROGRESS)) AS AVX FROM SUBTASKS WHERE (STSK_CHARGE_USR <> STSK_MAIN_USR AND STSK_TICKET = '" . $ticket . "' AND STSK_FAC_CODE = " . $fac. ")"));
              mysqli_query($datos, "UPDATE SUBTASKS SET STSK_PROGRESS = " . $avg['AVX'] . " WHERE (STSK_ISS_ID = " . $iss_id . " AND STSK_TYPE = 1 AND STSK_MAIN_USR = " . $muser . ")");
