                                        ...
                                        if($handler2 = opendir("../" . $_SESSION['TxtFacility'] . "/" . $boss['BOSS'] . "/" )){
                                          ...
                                          echo "<script>console.info('archivo :" . $archivos2 . " / value : " . preg_match_all("/_\[" . $stsk[1] . "\]_/", $archivos2) . "')</script>";
                                            if(preg_match_all("/_\[" . $stsk[1] . "\]_" . $_SESSION['TxtCode'] . "/", $archivos2) == 1){
                                              ...
                                              if(strlen($archivos2) > 4){
                                          ?>
  <a href="../<? echo $_SESSION['TxtFacility'] ?>/<? echo $boss['BOSS'] ?>/<? echo $archivos2 ?>" download title="<? printf($archivos2) ?>" > <i class="fa fa-file-<? printf($file_extension) ?>o fa-2x"  style="color: <? printf($cor) ?> "></i></a>
                                                  <? 
                                                  }
                                                }
                                              } // while false
...
$spec_tem = mysqli_query($datos, "SELECT CONCAT(A.USR_NAME , ' ',  A.USR_SURNAME), A.USR_ID, B.STSK_STATE, B.STSK_PROGRESS, B.STSK_RESP, B.STSK_CHARGE_USR, B.STSK_MAIN_USR, B.STSK_OVER, A.USR_RANGE FROM USERS A INNER JOIN SUBTASKS B ON(A.USR_ID = B.STSK_CHARGE_USR) WHERE (STSK_ISS_ID = " . $stsk[1] . "  AND STSK_TYPE = 0);");
...
<? } ?>
             ...
             <?  $part = mysqli_query($datos, "SELECT A.STSK_CHARGE_USR, CONCAT(B.USR_NAME, ' ', B.USR_SURNAME), A.STSK_ID, A.STSK_PROGRESS, A.STSK_ISS_ID, A.STSK_TICKET FROM SUBTASKS A INNER JOIN USERS B ON(B.USR_ID = A.STSK_CHARGE_USR) WHERE (STSK_TYPE= 1 AND STSK_ISS_ID =" . $fila5[11] . " AND STSK_CHARGE_USR <> STSK_MAIN_USR)"); 
...
//echo "<script>console.info('tester : " . $archivos2 . "' + ' / ' + '" . preg_match_all("/_\[" . $fint[2]. "\]_/", $archivos2) . "' + '/' + '" . $fint[2] . "' )</script>";
                                         if(preg_match_all("/_\[" . $fint[2] . "\]_/", $archivos2) == 1){
                                          ...
                                          ?>
                                              ...
                                              // closedir($handler2);
                                          mysqli_data_seek($part, 0);
                          //ping to db if min is sadmin, if is... check if htere are files to grow up and show.
                       while($fint = mysqli_fetch_row($part)){
...
<? } ?>
                          ...
                          <?    if(!is_dir("../" . $_SESSION['TxtFacility'] . "/" . $ii[1] . "_alt/")) {
                                    ...
                                    mkdir("../" . $_SESSION['TxtFacility'] . "/" . $ii[1] . "_alt/", 0775, true); 
                                        ...
                                        if($handler2 = opendir("../" . $_SESSION['TxtFacility'] . "/" . $ii[1] . "_alt/" )){
               ...
               //si no está vacio
                 if(parseInt(packets[0]) !== 0 && packets[0] !== "" ){
                       ...
                       //si es de tipo externo ==*.*==
                            ...
                            console.info('indice como resultado :' + indice);
