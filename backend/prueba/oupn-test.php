...
$Query_subtask    = mysqli_query($datos, "SELECT A.ISS_ID, A.ISS_ID , A.ISS_DESCRIP, B.EST_DESCRIPT, A.ISS_FINISH_DATE, B.EST_COLOR , A.ISS_PROGRESS, A.ISS_LOCK, ISS_TICKET FROM ISSUES A INNER JOIN EST B ON (B.EST_CODE = A.ISS_STATE) WHERE (ISS_FAC_CODE = " . $_SESSION['TxtFacility'] . " AND ISS_CHARGE_USR = " . $_SESSION['TxtCode'] . ")" );
...
$Query_alerts_int = mysqli_query($datos, "SELECT COUNT(STSK_ID), STSK_STATE FROM SUBTASKS WHERE (STSK_FAC_CODE = " . $_SESSION['TxtFacility'] . " AND STSK_MAIN_USR = " . $_SESSION['TxtCode'] . " AND STSK_CHARGE_USR = STSK_MAIN_USR AND STSK_TYPE = 0) GROUP BY STSK_STATE");
$Query_alerts_ii  = mysqli_query($datos, "SELECT COUNT(STSK_ID), STSK_STATE FROM SUBTASKS WHERE (STSK_CHARGE_USR = " . $_SESSION['TxtCode'] . " AND STSK_CHARGE_USR <> STSK_MAIN_USR AND STSK_LOCK = 1 AND STSK_TYPE = 0) GROUP BY STSK_STATE");
...
$query_internal = "SELECT A.ISS_ID, A.ISS_CHARGE_USR, CONCAT(B.USR_NAME, ' ' , B.USR_SURNAME), A.ISS_CHARGE_USR, A.ISS_SUBJECT, A.ISS_DESCRIP, C.EST_DESCRIPT, A.ISS_PROGRESS, C.EST_COLOR,  A.ISS_LOCK, A.ISS_FINISH_DATE, A.ISS_ID , A.ISS_TICKET FROM ISSUES A INNER JOIN USERS B ON(A.ISS_CHARGE_USR = B.USR_ID) INNER JOIN EST C ON(C.EST_CODE = A.ISS_STATE) WHERE (ISS_CHARGE_USR = " . $_SESSION['TxtCode'] . " AND ISS_FAC_CODE = " . $_SESSION['TxtFacility'] . ")";
                                        ...
                                        if($handler2 = opendir("../" . $_SESSION['TxtFacility'] . "/" . $fint[0] . "/" )){
                                         ...
                                         if(preg_match_all("/_" . ($fint[4]) . "_/", $archivos2) == 1){
                                          ...
                                          ?>
                                                 ...
                                                 <a href="../<? printf($_SESSION['TxtFacility']) ?>/<? echo $_SESSION['TxtCode'] ?>/<? printf($archivos2) ?>" download>
