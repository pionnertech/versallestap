...
$over = mysqli_fetch_assoc(mysqli_query($datos, "SELECT ISS_CHARGE_USR AS SW FROM ISSUES WHERE ISS_ID = " . $iss_id));
// ticket  plus
...
if($user == "Mi Departamento"){
    ...
    while($fila = mysqli_fetch_row($uteam)){
        $query = "INSERT INTO SUBTASKS (STSK_ISS_ID, STSK_SUBJECT, STSK_DESCRIP ,STSK_CHARGE_USR, STSK_FINISH_DATE, STSK_STATE, STSK_START_DATE, STSK_MAIN_USR, STSK_FAC_CODE, STSK_PROGRESS, STSK_TYPE, STSK_LOCK, STSK_TICKET, STSK_OVER) ";
        $query .= "VALUES (" . $iss_id . ", '" . $subject . "', '" . $descript . "', " . $fila[0] . ", '" . $fechaF . "', 1 ,  '" . $startD . "' , " . $muser . ", " . $fac . ", 0, 0, 1, '" . $ticket['STSK_TICKET']. "', " . $sr .")";
...
//transform user name 
  ...
  for($i= 0; $i < count($uteam_array); $i++ ){
       ...
       $query = "INSERT INTO SUBTASKS (STSK_ISS_ID, STSK_SUBJECT, STSK_DESCRIP ,STSK_CHARGE_USR, STSK_FINISH_DATE, STSK_STATE, STSK_START_DATE, STSK_MAIN_USR, STSK_FAC_CODE, STSK_PROGRESS, STSK_TYPE, STSK_LOCK, STSK_TICKET, STSK_OVER) ";
       $query .= "VALUES (" . $iss_id . ", '" . $subject . "', '" . $descript . "', '" . $usr_id_q['USR_ID'] . "', '" . $fechaF . "', 1 ,  '" . $startD . "' , '" . $muser . "', " . $fac . ", 0, 0, 1, '" . $ticket['STSK_TICKET'] . "', " . $sr .")";
  	   mysqli_query($datos, $query);
